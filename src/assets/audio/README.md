# Audio Assets

## `battle_music_01-loop.ogg`

- Title: Chiptune Battle Music
- Author: pmiller
- Source: https://opengameart.org/content/chiptune-battle-music
- License: CC0
- Notes: The source page states the loop is free to use without attribution.

## `menu_music_loop.ogg`

- Title: Menu Music
- Author: wipics
- Source: https://opengameart.org/content/menu-music-1
- Source file: `menu.ogg`
- License: CC0
- Notes: Public Domain menu loop used outside active duels.

## Card Battle SFX

The following short effects are edited from card-focused CC0 packs for card
selection and card play feedback.

- `sfx-card-play.ogg`
  - Source asset: `card-place-2.ogg`
  - Source pack: Kenney Casino Audio
  - Source: https://opengameart.org/content/54-casino-sound-effects-cards-dice-chips
- `sfx-select.ogg`
  - Source asset: `playcard.wav`
  - Source pack: Card Game Sounds
  - Source: https://opengameart.org/content/card-game-sounds
- `sfx-card-hover.ogg`
  - Source asset: `draw.wav`
  - Source pack: Card Game Sounds
  - Source: https://opengameart.org/content/card-game-sounds
- `sfx-card-draw.ogg`
  - Source asset: `card-slide-2.ogg`
  - Source pack: Kenney Casino Audio
  - Source: https://opengameart.org/content/54-casino-sound-effects-cards-dice-chips
- `sfx-charge.ogg`
  - Source asset: `Rise02.aif`
  - Source pack: Level Up, Power Up, Coin Get (13 Sounds)
  - Sources:
    - https://opengameart.org/content/level-up-power-up-coin-get-13-sounds
- License: Creative Commons CC0

## Battle SFX

The following short effects are used for command, trash, and turn-end feedback.

- `sfx-command.ogg`
- `sfx-trash.ogg`
- `sfx-turn-end.ogg`
- Source pack: Kenney Interface Sounds
- Source: https://kenney.nl/assets/interface-sounds
- License: Creative Commons CC0

The following impact effects are used for attack, block, and damage feedback.

- `sfx-attack.ogg`
- `sfx-block.ogg`
  - Source asset: `impactPlate_medium_000.ogg`
- `sfx-damage.ogg`
  - Source asset: `impactMining_002.ogg`
- Source pack: Kenney Impact Sounds
- Source: https://kenney.nl/assets/impact-sounds
- License: Creative Commons CC0
