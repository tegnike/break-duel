...
function AttackDefensePanel({ game, forcedDefenseChoice, onResolve }: { game: GameState; forcedDefenseChoice?: DefenseChoice | null; onResolve: (choice: DefenseChoice) => void }) {
  ...
  const strikeInfo = pending.strikeTargetIndex !== undefined && strikeTarget
    ...
    : null;
  const fieldOptions = strikeTarget ? [] : legalFieldDefenders(defender, attackCard, attackContext);
  ...
  return (
    <div className="defense-panel">
      ...
      <div className="defense-choice-grid">
        {visibleFieldOptions.map(({ card, index }) => <DefenseChoiceButton key={`field-${index}`} source="場" card={card} cardIndex={index} attackCard={attackCard} attackContext={attackContext} defender={defender} fieldIndex={index} firewallEnabled={firewallEnabled} onResolve={onResolve} />)}
...
function DefenseChoiceButton({ source, card, cardIndex, attackCard, attackContext, defender, hand = false, fieldIndex, firewallEnabled, onResolve }: { source: string; card: Card; cardIndex: number; attackCard: Card; attackContext: AttackContext; defender: PlayerState; hand?: boolean; fieldIndex?: number; firewallEnabled: boolean; onResolve: (choice: DefenseChoice) => void }) {
