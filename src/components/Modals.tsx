...
const RULE_SECTIONS = [
  {
    ...
    items: [
      ...
      `${CONFIG.maxTurns} 手番で決着しない場合は、ライフが多い側の勝利。同値のみ引き分け。`,
    ],
  },
  {
    title: "ターン",
    items: [
      ...
      `ターン開始時に山札が空でドローできない場合、衰弱で ${CONFIG.deckOutFatigueDamage} ダメージを受けます。`,
      `自分のターン終了時、手札が ${CONFIG.handLimit} 枚を超えていれば超過分を自動でトラッシュします。`,
    ],
  },
  {
    title: "カード種と属性",
  ...
  },
  {
    ...
    items: [
      ...
      "防御しなかった攻撃は、攻撃した召喚獣の power と同じダメージを与えます（power 1〜4 = 1〜4 点）。",
      "「戦闘時、攻撃値 +1」などの補正は戦闘（防御や討伐の判定）でのみ使われ、ダメージは power で決まります。",
      "攻撃ダメージを受けた側は、受けた点数分カードを引きます（ブレイクドロー）。",
      "攻撃対象には相手プレイヤーのほか、相手の召喚獣も選べます。召喚獣を直接狙う場合、上回れば討伐、同値なら相打ちです。",
      "場防御は同値なら相打ち、上回れば攻撃側だけトラッシュ。防御側は場に残って消耗します。プレイヤーへの攻撃では、防御値が不足しても選べますが、防御側はトラッシュされ、攻撃値との差分ダメージを受けます。",
      `自分の召喚獣への攻撃は、場の別召喚獣でかばうか、手札の召喚獣で防御できます。手札防御は power ${CONFIG.handDefenseMaxPower} 以下・1 ターンに 1 回で、防御カードはトラッシュへ行きます（回数は共通）。`,
