...
5. **計測器（challenger CPU）の世代を確認する**: 全勝率数値は CPU 世代（`docs/assets/ai-champions/fair/`
   ...
   CPU 自体を変更したくなったら `.agents/skills/ai-break-duel-cpu-improvement/SKILL.md` に切り替える
...
- **CPU（challenger/beginner）の変更が主目的の作業は、専用スキル
  `.agents/skills/ai-break-duel-cpu-improvement/SKILL.md` に従うこと**（ガントレット世代管理・
  公平性ガードテスト・55% ゲート・計画書駆動のターム運用はそちらが正）。
  本節はカード/ルール変更のついでに CPU 較正を確認する場合の簡易手順にすぎない
