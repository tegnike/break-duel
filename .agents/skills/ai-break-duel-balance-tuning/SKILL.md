...
5. **計測器（challenger CPU）の世代を確認する**: 全勝率数値は CPU 世代（`docs/assets/ai-champions/fair/fair-gen006.json`）
   に紐づく。**CPU 世代が違う時代の数値と比較してはいけない**（覗き見時代・
   旧世代時代の数値は現行と互換性がない）。基準は fair-gen006 で固定した
   balance-history 最新エントリの再ベースライン値。
