...
React + TypeScript のブラウザ UI で実際に対戦でき、同じ TypeScript エンジンの CLI で自動対戦とバランス検証を回せます。
...
- ブラウザ UI では使用デッキとCPU難度を持つ対戦キャラクターを複数登録し、1試合ごとに1人を選べます。
- `キャラ管理` では対戦キャラクターを追加し、通常・被弾・喜び・切札・とどめの各イラストと、15イベント分のセリフ・音声を登録できます。
- 相手 CPU はプロフィールごとに `初心者` と `挑戦者` から選べます。`挑戦者` は自動探索した評価関数で合法手を比較します。
- TypeScript CLI で大量の自動対戦、リーグ戦、ルール実験を実行できます。
...
対戦準備では、自分のデッキと登録済み対戦キャラクターだけを選びます。相手のデッキとCPU難度はキャラクター管理で設定した値を使用し、対戦準備からは変更できません。キャラクターの作成・編集・削除も `キャラ管理` へ一本化しています。

## キャラクター管理

`/admin/characters` の `キャラ管理` 画面では、対戦キャラクターの表示名とID、使用デッキ、CPU難度、5種類のイラスト、15種類のゲームイベントに対応する表示セリフと音声ファイルを登録・編集・削除できます。通常立ち絵だけが必須で、未登録のリアクション画像は通常立ち絵へフォールバックします。

登録した画像・音声は容量の小さい `localStorage` ではなくIndexedDBへ保存され、再読み込み後も同じブラウザで利用できます。保存直後から対戦相手プロフィールのキャラクター選択肢へ追加されます。現在はバックエンドやアカウント同期を持たないため、登録内容はブラウザ・オリジン単位のローカル管理です。

プロフィールは `localStorage` の `break-duel:opponent-profiles` にversion付きで保存されます。削除済みの保存デッキなどを参照するプロフィールは勝手に置き換えず「要修正」と表示し、修正するまで対戦開始を止めます。進行中の試合は開始時の相手スナップショットを使うため、次戦向けプロフィールの編集や削除では変化しません。
