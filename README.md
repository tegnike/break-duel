...
- モンスター攻撃: 相手の召喚獣を攻撃対象に選べる（場防御と同じ判定式）。防御側は手札防御で割り込める
...
モンスター攻撃への手札防御割り込みを無効化（2026-07-05 以前のルール）して比較したい場合は `--hand-defense-vs-strike off` を付けます（標準は `value`、検証用に `eager` も選択可）。
