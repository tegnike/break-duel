...
- `src/game.ts`: TypeScript 側カード定義、設定、純粋ルール、AI判断
...
- 新規対戦が開始され、先攻初期手札 5 枚 + 初回ドロー 1 枚が表示される。
- 手札の AI を選択して場に出せる。
...
- `src/App.tsx` に画面操作とReactコンポーネント、`src/game.ts` にゲームルールとAI判断を置く。
...
- power 4 は攻撃後オーバーヒートする。
- サンドボックス使用後の power 4 は1回だけ場に残る。
- トラッシュモーダル内に効果が表示される。
...
- 防御候補表示は `defenseCombatValue` と同じ計算を使う。
- ブラウザキャッシュ対策は Vite のハッシュ付きビルドに任せる。
...
- 攻撃、防御、指令、メモリー、アップグレード、交換、トラッシュ閲覧が動く。
...
- 防御候補表示、サンドボックス指令、先攻初ターン攻撃不可を壊さない
- アニメーションと効果音は最後に戻す
