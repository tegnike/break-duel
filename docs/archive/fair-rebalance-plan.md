...
前提計画: `docs/archive/fair-cpu-plan.md`（完了）→ `docs/archive/fair-cpu-results.md`（結果）→ `docs/fair-cpu-followups.md`（課題の一次記録）
...
> 2. 基準数値は `docs/archive/fair-cpu-results.md` の F1 再ベースライン値（本書 §1 に転記）。
...
> 4. コマンドは `docs/archive/fair-cpu-results.md` 末尾の検証コマンド一覧と `docs/archive/strongest-cpu-plan.md` §10 を使う
...
2. 原因に応じて: beginner の弱体化（挙動の単純化・意図的な悪手率）または
   ...
   ガントレット必須で `docs/archive/fair-cpu-plan.md` の作法に従う）
