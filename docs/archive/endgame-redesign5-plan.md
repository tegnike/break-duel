...
前提: 第一〜四弾すべて完了。第四弾（`docs/archive/endgame-redesign4-plan.md`）で残った不合格は
...
- [x] 0-1. 正本ブランチ確認 + 未コミット差分の確認（あればコミット）→ 状態:
      ...
      本計画書 `docs/archive/endgame-redesign5-plan.md` の未追跡のみ。別 worktree
...
- [x] 0-3. 第四弾の最終数値（§0 の表）と土台デッキ変更 3 件を確認 → 確認:
      `docs/archive/endgame-redesign4-plan.md` の Step 4 / §8 で、合格帯
