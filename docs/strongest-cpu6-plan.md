...
## 8. 追補（2026-07-13）: 第 2 の真因「対人先読みの攻撃未解決」

エンジン修正後の実対戦で「蒼殻バリア発動後、アクション 2 残しでミストラル不攻撃」が報告され、
追加診断で `beginAttackInDraft` / `strikeInDraft` が人間防御側で停止する仕様が、
ビーム先読みのドラフトでも発動して**攻撃プランの戦果を終端評価から消していた**ことを特定した。
CPU 同士のシミュレーションでは検出不能な対人戦専用欠陥であり、旧 4 戦の「対人攻撃ダメージ 0」の主因。
`planningDraft` フラグで先読み中のみ公開情報の防御推定により解決する修正を採用
（詳細と検証は balance-history 2026-07-13 エントリ）。§7 に残課題として挙げた「受け身」の大半はこれで解消し、
再現では攻撃系選択が 3→10 へ増加、2 手詰めリーサルの発見も可能になった。
